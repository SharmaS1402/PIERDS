...
if img is not None:
    print('The image is valid and not corrupted.')
else:
    print('The image is corrupted or cannot be read.')
...
class Face:
    ...
    def save_face_info(self, names, face_images):
        if type(names) == list and type(face_images) == list:
          for name, face_image in zip(names, face_images):
            ...
            if face_encodings:
                self.known_face_encoding.append(face_encodings[0])
            else:
                print("No face encodings found for", name)
        else:    
            self.face_name.append(names)
            self.known_face.append(face_images)
            face_encodings = fr.face_encodings(face_images)
            if face_encodings:
               self.known_face_encoding.append(face_encodings[0])
            else:
               print("No face encodings found for", names)
...
if __name__ == '__main__':
    ...
    img = cv2.imread('face/people.jpeg')
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    names, face_images = bruh.get_face_info(img, False)
    print(names)
    # names, face_images = bruh.get_face_info(img, True)
    # print(names)
    for name, face_image in zip(names, face_images):
